...
export class DictionaryService {
  private static lastKnown = new Map<number, DictionaryEntry[]>();
  static async listForSearch(empresaId: number): Promise<DictionaryEntry[]> {
    return OptionalSearchDependency.run(`dictionary:${empresaId}`, async () => {
      const entries = await this.listActive(empresaId);
      this.lastKnown.set(empresaId, entries);
      return entries;
    }, this.lastKnown.get(empresaId) || []);
  }
